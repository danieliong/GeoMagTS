...
double * tf_admm_default(double * y, int n)
{
  ...
  lam = (double *) malloc(nlambda * sizeof(double));
  ...
  tf_admm(x, y, w, n, k, family, max_iter, lam_flag, lam,
 ...
 * @param lam               either a sequence of lambda when lam_flag=0, or empty
    ...
    int max_iter_ls, int max_iter_newton, int verbose)
{
   ...
   * equally spaced points in log space.
	 */
  if (!lam_flag)
		genInLogspace(max_lam,lambda_min_ratio,nlambda,lam);

  /* Initiate alpha and u for a warm start */
  if (lam[0] < max_lam * 1e-5)  
    ...
    /* u_max */
		for (i = 0; i < n; i++) 
			u[i] = w[i] * (beta_max[i] - y[i]) / (rho * lam[0]);
  ...
  for (i = 0; i < nlambda; i++)
  {
    ...
    switch (family)
    {
      case FAMILY_GAUSSIAN:
        tf_admm_gauss(x, y, w, n, k, max_iter, lam[i], df+i, beta+i*n,
            alpha, u, obj+i*max_iter, iter+i, rho * lam[i],
            obj_tol, DktDk, verbose);
        break;

      case FAMILY_LOGISTIC:
        tf_admm_glm(x, y, w, n, k, max_iter, lam[i], df+i, beta+i*n,
            alpha, u, obj+i*max_iter, iter+i, rho * lam[i], obj_tol,
    ...
    if (has_nan(beta + i*n, n))
    {
      ...
      for (j = 0; j < n; j++) u[j] = w[j] * (beta[i*n+j] - y[j]) / (rho * lam[i]);
      glmgen_qrsol (Dkt_qr, u);
      status[i] = 1;
    }
  }
